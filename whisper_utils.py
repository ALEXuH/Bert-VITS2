...
class WhisperTool:
    ...
    def process_directory(self, directory_path, language="zh") -> None:
        ...
        try:
            ...
            logger.error(
                f"Whisper transcribe error occurred while processing whisper_save_txt: {str(e)}"
            )
    ...
    def process_file(self, file_path, language="zh") -> None:
        ...
        try:
            ...
            with open(
                os.path.join(output_directory, file_prefix + ".lab"), "w"
            ) as file:
                v = []
            ...
            logger.error(
                f"Whisper: An error occurred while processing whisper_save_txt: {str(e)}"
            )
            logger.error(traceback.format_exc())
